...
# 
# 
#   synth_eq <- CO2..tons. ~ inputed+ tmmx + rmax + pr + vs #omiting wind direction
# 
...
# close(fileConn)
# 
# save(result_co2_weekly_2020, file="data/result_co2_weekly_2020_no_wd.RData")
load ("data/result_co2_weekly_2020_no_wd.RData")
load ("data/facility_co2_2020.RData")
...
pdf(file = 'plots/counterfactual_plots_co2_2020_no_wd.pdf', onefile = TRUE, paper = 'A4',
...
for (i in 1:length(facility)) {
  ...
  att.avg <-result_co2_weekly_2020[[i]][[2]]$att
...
names(all.facility.co2.2020)[names(all.facility.co2.2020) == 'att.avg'] <- 'ATT'
