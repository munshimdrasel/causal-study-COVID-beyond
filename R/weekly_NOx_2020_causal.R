...
#   all_ampd <-  all_ampd %>%  dplyr::select (STATE, year, week, ORISPL_CODE, ID, NOx..tons.,
#                                             pr, tmmx, rmax, vs, th)
...
# 
#   all_ampd_final <- all_ampd%>% dplyr::select (year, week, id, NOx..tons.,  inputed,
#                                                pr, tmmx, rmax, vs, th)
# 
# 
...
#   return(all)
# }
# result_nox_weekly_2020 <- lapply(facility, gsynth.fn)
# close(fileConn)


# save(result_nox_weekly_2020, file="data/result_nox_weekly_2020.RData")
load ("data/result_nox_weekly_2020.RData")
load ("data/facility_nox_2020.RData")
...
#   if (length(sublist) == 3)
#   {
#     result_nox_weekly_2020_test[[i]] <-result_nox_weekly_2020[[i]] # this does achieve the desired result
...
for (i in 1: length(facility)) {
  counterfactual_plots[[i]] <- result_nox_weekly_2020 [[i]][[3]]
}
layout <- rbind(c(1,2), c(3,4), c(5,6))
pdf(file = 'plots/counterfactual_plots_nox_2020.pdf', onefile = TRUE, paper = 'A4',
...
for (i in 1:length(facility)) {
  ...
  att.avg <-result_nox_weekly_2020[[i]][[2]]$est.att
  week <- 1:nrow(att.avg)
...
all.facility.nox.2020 <- merge (st, ampd_facility, by = "facility")
...
for (i in 1:length(facility)) {
  nox.emis <- na.omit(as.data.frame(result_nox_weekly_2020[[i]][[2]]$Y.bar))
  nox.emis <- nox.emis %>% dplyr::select(-Y.co.bar)
  nox.emis$week <- c(1:nrow(nox.emis)) #leap year 2020
  fac.1 <- as.data.frame(facility[i])
