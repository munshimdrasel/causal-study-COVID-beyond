...
# setwd ("/Users/munshirasel/Desktop/HAQ_LAB/R/causal-study")

#getting all electric facilities in the US
ampd_daily_all_units<- read.fst ("data/ampd_daily_cleaned_facility_co2.fst")
...
gsynth.fn <- function(facility.name) {
  ...
  all_ampd <-  all_ampd %>%  dplyr::select (date, STATE, year, month, day, ORISPL_CODE, ID, CO2..tons., 
  ...
  all_ampd_final <- all_ampd%>% dplyr::select (year, time, id, CO2..tons.,  inputed,
                                               pr, tmmx, rmax, vs, th,
                                               weekdays, inputed,season)
  
  #factor doesn't work with gsynth??
  #all_ampd$feb <- as.numeric(all_ampd$feb)
  synth_eq <- CO2..tons. ~ inputed+ tmmx + rmax + pr + th + vs +weekdays
...
result_co2_whole_2020 <- lapply(facility, gsynth.fn)
close(fileConn)

#saving file
save(result_co2_whole_2020, file="data/result_facility_co2_2020_whole.RData")

# load ("data/result_facility_co2_2020_whole.RData")
...
# 
# for (i in 1: length(facility)) {
#   counterfactual_plots[[i]] <- result [[i]][[3]]
# }
# 
# 
# layout <- rbind(c(1,2,3), c(4,5,6), c(7,8,9))
# pdf(file = 'counterfactual_plots_so2.pdf', onefile = TRUE, paper = 'A4', 
...
# # dev.off()
# 
# st <- as.data.frame(matrix(nrow = 1000, ncol = 19))
# 
# for (i in 1:length(facility)) {
#   rs.1 <- as.data.frame((result[[i]][[1]]))
#   fac.1 <- as.data.frame(facility[i])
#   att.avg <- as.data.frame(result[[i]][[2]]$est.avg)
#   cv <- as.data.frame(result[[i]][[2]]$lambda.cv)
#   mspe <- as.data.frame(result[[i]][[2]]$MSPE)
...
# st <- st %>% select (-V2)
# 
# old.names <- c ("V1",  "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13", "V14", "V15", "V16", "V17", "V18", "V19")
# new.names <- c ("facility", "n","FAC2", "MB", "MGE", "NMB", "NMGE", "RMSE", "r", "COE", "IOA", "ATT", "S.E", "lower.ci", "upper.ci", "p.value", "lambda.cv", "MSPE" )
# 
...
#   
#   ampd_daily_all_unit.op <- ampd_daily_all_unit %>% mutate(op.pct =(length(ampd_daily_all_unit$CO2..tons. [ampd_daily_all_unit$CO2..tons. >0])/
#                                                                       length(ampd_daily_all_unit$CO2..tons. ))*100)
#   ampd_daily_all_unit <- ampd_daily_all_unit.op %>% dplyr::select(fips, STATE, ORISPL_CODE, Fuel.Type..Primary.,
...
# 
# operation <- as.data.frame(matrix(nrow = 26, ncol =8))
# 
# for (i in 1:length(facility)) {
#   operation [i, ] <- as.data.frame((op[i]))
# }
# 
# old.names <- c ("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8")
# new.names <- c ("fips", "STATE","facility", "Fuel.Type..Primary.", "Facility.Latitude", "Facility.Longitude", 
#                 "County", "op.pct" )
# 
...
# 
# write.fst (all.facility, "data/metrics_facility_so2.fst")
# 
# 
