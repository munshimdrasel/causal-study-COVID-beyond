...
#   
#   all_ampd_final <- all_ampd%>% dplyr::select (year, week, id, inputed, SUM_OP_TIME,
#                                                pr, tmmx, rmax, vs, th)
#   
#   
#   synth_eq <- SUM_OP_TIME ~ inputed+ tmmx + rmax + pr + vs
#   
...
#   return(all)
# }
# result_SUM_OP_TIME_weekly_2020 <- lapply(facility, gsynth.fn)
# close(fileConn)
# 
# 
# save(result_SUM_OP_TIME_weekly_2020, file="data/result_SUM_OP_TIME_weekly_2020.RData")

load ("data/result_SUM_OP_TIME_weekly_2020.RData")
load ("data/facility_nox_2020.RData")
...
for (i in 1: length(facility)) {
  counterfactual_plots[[i]] <- result_SUM_OP_TIME_weekly_2020[[i]][[3]]
}
layout <- rbind(c(1,2), c(3,4), c(5,6))
pdf(file = 'plots/counterfactual_plots_op_time_2020.pdf', onefile = TRUE, paper = 'A4',
...
for (i in 1:length(facility)) {
  ...
  att.avg <-result_SUM_OP_TIME_weekly_2020[[i]][[2]]$att
...
all.facility.op.time.2020 <- merge (st, ampd_facility, by = "facility")
...
for (i in 1:length(facility)) {
  op.time <- na.omit(as.data.frame(result_SUM_OP_TIME_weekly_2020[[i]][[2]]$Y.bar))
  op.time <- op.time %>% dplyr::select(-Y.co.bar)
  op.time$week <- c(1:nrow(op.time)) #leap year 2020
  fac.1 <- as.data.frame(facility[i])
  datalist[[i]] <- cbind(fac.1,op.time) # add it to your list
}

ac.ct.op.time= do.call(rbind, datalist)

names(ac.ct.op.time)[names(ac.ct.op.time) == 'facility[i]'] <- 'facility'

all.facility.op.time.2020 <- merge (all.facility.op.time.2020, ac.ct.op.time, by = c( "facility", "week" ))

names(all.facility.op.time.2020)[names(all.facility.op.time.2020) == 'att.avg'] <- 'ATT'

write.fst(all.facility.op.time.2020, "data/all.facility.op.time.2020.fst")
