...
#   
#   all_ampd_final <- all_ampd%>% dplyr::select (year, week, id, inputed, HEAT.INPUT,
#                                                pr, tmmx, rmax, vs, th)
#   
#   
#   synth_eq <-  HEAT.INPUT ~ inputed+ tmmx + rmax + pr + vs
#   
...
#   return(all)
# }
# result_HEAT.INPUT_weekly_2020 <- lapply(facility, gsynth.fn)
# close(fileConn)
# 
# 
# save(result_HEAT.INPUT_weekly_2020, file="data/result_HEAT.INPUT_weekly_2020.RData")

load ("data/result_HEAT.INPUT_weekly_2020.RData")
load ("data/facility_nox_2020.RData")
...
#getting counterfactua plots for 2020 heat input
counterfactual_plots <- list()
for (i in 1: length(facility)) {
  counterfactual_plots[[i]] <- result_HEAT.INPUT_weekly_2020 [[i]][[3]]
}
layout <- rbind(c(1,2), c(3,4), c(5,6))
pdf(file = 'plots/counterfactual_plots_heat_input_2020.pdf', onefile = TRUE, paper = 'A4',
...
for (i in 1:length(facility)) {
  ...
  att.avg <-result_HEAT.INPUT_weekly_2020[[i]][[2]]$att
...
all.facility.heat.input.2020 <- merge (st, ampd_facility, by = "facility")
...
for (i in 1:length(facility)) {
  heat.input <- na.omit(as.data.frame(result_HEAT.INPUT_weekly_2020[[i]][[2]]$Y.bar))
  heat.input <- heat.input %>% dplyr::select(-Y.co.bar)
  heat.input$week <- c(1:nrow(heat.input)) #leap year 2020
  fac.1 <- as.data.frame(facility[i])
  datalist[[i]] <- cbind(fac.1,heat.input) # add it to your list
}

ac.ct.heat.input = do.call(rbind, datalist)

names(ac.ct.heat.input)[names(ac.ct.heat.input) == 'facility[i]'] <- 'facility'

all.facility.heat.input.2020 <- merge (all.facility.heat.input.2020, ac.ct.heat.input, by = c( "facility", "week" ))

names(all.facility.heat.input.2020)[names(all.facility.heat.input.2020) == 'att.avg'] <- 'ATT'

write.fst(all.facility.heat.input.2020, "data/all.facility.heat.input.2020.fst")
