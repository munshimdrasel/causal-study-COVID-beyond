...
for (i in 1:length(facility)) {
  ...
  rs.1 <- rs.1 %>% select(ORISPL_CODE, CO2..tons.,NOx..tons.)
  rs.2 <- setDT(rs.1)[, .(op.pct = sum(!is.na(rs.1$CO2..tons.))/length(rs.1$NOx..tons.)*100),
                      by = .(ORISPL_CODE)]
...
write.fst(ampd_daily_all_units, "data/ampd_daily_cleaned_facility_co2.fst")
